...
/**
 * =============================================================================
 * Logger — SimplifyTable diagnostic logger
 * =============================================================================
 *
 * JSON-line structured logging to a daily-rotated file.
 *   - daily rotation (file name = simplifytable-YYYY-MM-DD.log)
...
class Logger
  ...
  private static function write(string $level, string $context, array $data): void
    ...
    try {
      ...
      $file = $dir . DIRECTORY_SEPARATOR . 'simplifytable-' . date('Y-m-d') . '.log';
  ...
  private static function purgeOld(string $dir): void
    ...
    $files = @glob($dir . DIRECTORY_SEPARATOR . 'simplifytable-*.log*') ?: [];
    foreach ($files as $f) {
      $mtime = @filemtime($f);
